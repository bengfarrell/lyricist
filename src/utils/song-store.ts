...
export class SongStore {
  ...
  private _currentPanel: 'word-ladder' | 'canvas' | 'lyrics' = 'canvas';
  private _stripRetracted: boolean = false;

  // Canvas pan state (for infinite canvas)
  private _canvasPanX: number = 0;
  private _canvasPanY: number = 0;
  private _isSpacebarPanMode: boolean = false;
  ...
  private _wordLadderRandomColumnCount: number = 2; // Number of columns to use in randomizer
  ...
  get showWordLadderConfig(): boolean {
    return this._showWordLadderConfig;
  }
  ...
  get currentPanel(): 'word-ladder' | 'canvas' | 'lyrics' {
  ...
  get canvasPanX(): number {
    return this._canvasPanX;
  }

  get canvasPanY(): number {
    return this._canvasPanY;
  }

  get isSpacebarPanMode(): boolean {
    return this._isSpacebarPanMode;
  }
  ...
  get wordLadderRandomColumnCount(): number {
    return this._wordLadderRandomColumnCount;
  }
  ...
  setShowWordLadderConfig(show: boolean): void {
    this._showWordLadderConfig = show;
    this.notify();
  }
  ...
  setCurrentPanel(panel: 'word-ladder' | 'canvas' | 'lyrics'): void {
  ...
  deleteSelectedItems(): void {
    const selectedIds = Array.from(this._selectedLineIds);
    ...
    // Selection is already cleared by individual delete methods
  ...
  adjustCanvasPan(deltaX: number, deltaY: number): void {
    this._canvasPanX += deltaX;
    this._canvasPanY += deltaY;
    this.notify();
  }

  setSpacebarPanMode(enabled: boolean): void {
    this._isSpacebarPanMode = enabled;
    this.notify();
  }
  ...
  // Method to set random column count
  setWordLadderRandomColumnCount(count: number): void {
    this._wordLadderRandomColumnCount = Math.max(1, Math.min(count, this._wordLadderColumns.length));
    this.notify();
  }
